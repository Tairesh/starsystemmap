...
THREEx.DomEvent.prototype.unbind	= function(object3d, eventName, callback, useCapture)
{
	...
	if(!(eventName in this._boundObjs)) return;
...
THREEx.DomEvent.prototype.gal = false;
THREEx.DomEvent.prototype._onMove	= function(eventName, mouseX, mouseY, origDomEvent)
{
	THREEx.DomEvent.prototype.mouseX = mouseX;
	THREEx.DomEvent.prototype.mouseY = mouseY;
//console.log('eventName', eventName, 'boundObjs', this._boundObjs[eventName])
	// get objects bound to this event
	if(this.gal && cScreen == 0) return;
	...
	if( intersects.length > 0 ){
		...
		var intersect	= intersects[ indexSelect ];
...
//console.log('eventName', eventName, 'boundObjs', this._boundObjs[eventName])
	...
	for (var i = 0; i < intersects.length; i++) {
		if(intersects[i].object.oid)
		{
			if(list[intersects[i].object.oid].type == "station")
				continue;
			else
			{
				indexSelect = i;
				if(intersects[i].object.action)
					break;
			}
		}
	};
